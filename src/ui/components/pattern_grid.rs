...
fn get_error_container_colors(visuals: &egui::Visuals) -> (egui::Color32, egui::Color32) {
    if visuals.dark_mode {
        ...
        (
            ...
            egui::Color32::from_rgb(220, 180, 180),
        )
    }
}
    ...
    pub fn show(
        ...
        egui::Frame::none()
            ...
            .stroke(egui::Stroke::new(
                1.0,
                get_pattern_grid_stroke_color(&ui.visuals()),
            ))
                    ...
                    ui.horizontal(|ui| {
                        ...
                        for step_index in 0..loop_length {
                            ...
                            let is_beat_boundary =
                                time_signature.is_beat_boundary(step_index, loop_length);
                            ...
                            if ui.add_sized([STEP_BUTTON_WIDTH, 32.0], button).clicked() {
                                ...
                                if let Ok(mut timeline) = timeline.try_lock() {
                                    if let Some(segment) =
                                        timeline.get_segment_mut(&segment_to_display)
                                    {
                                        if let Some(pattern) =
                                            segment.patterns.get_mut(pattern_index)
                                        {
                        ...
                        if ui
                            .add_sized([CLEAR_BUTTON_WIDTH, 32.0], clear_button)
                            .clicked()
                        {
...
mod tests {
    ...
    fn test_pattern_grid_visual_consistency() {
        ...
        let min_pattern_width = EXPECTED_TRACK_NAME_WIDTH
            + (16.0 * EXPECTED_STEP_BUTTON_WIDTH)
            + EXPECTED_CLEAR_BUTTON_WIDTH
            + (20.0 * EXPECTED_SPACING);
        assert!(
            min_pattern_width > 600.0,
            "Pattern grid minimum width should be reasonable: {}",
            min_pattern_width
        );
    }
