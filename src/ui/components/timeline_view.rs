...
impl TimelineView {
    ...
    fn draw_scrollable_timeline(&mut self, ui: &mut egui::Ui, rect: egui::Rect, _timeline: &Arc<Mutex<Timeline>>) {
        ...
        self.draw_time_ruler(&painter, rect, content_rect);
        ...
        self.draw_snap_grid(&painter, content_rect);
    ...
    fn draw_time_ruler(&self, painter: &egui::Painter, viewport_rect: egui::Rect, _content_rect: egui::Rect) {
    ...
    fn generate_pattern_preview(&self, segment: &TimelineSegment, preview_width: f32, preview_height: f32) -> Vec<(egui::Pos2, egui::Color32)> {
        ...
        let pattern_height = preview_height / segment.patterns.len().min(4) as f32; // Max 4 patterns visible
        
        for (pattern_idx, pattern) in segment.patterns.iter().enumerate().take(4) {
    ...
    fn draw_segment(&self, painter: &egui::Painter, rect: egui::Rect, segment: &TimelineSegment, _ui: &egui::Ui) {
        ...
        if segment_rect.width() > 40.0 && preview_area_height > 8.0 {
            let preview_elements = self.generate_pattern_preview(segment, preview_width, preview_area_height - 4.0);
    ...
    fn draw_snap_grid(&self, painter: &egui::Painter, rect: egui::Rect) {
...
mod tests {
    ...
    fn test_pattern_preview_generation() {
        ...
        let preview_elements = timeline_view.generate_pattern_preview(&segment, preview_width, preview_height);
    ...
    fn test_pattern_preview_with_empty_patterns() {
        ...
        // Generate pattern preview
        let preview_elements = timeline_view.generate_pattern_preview(&segment, 100.0, 20.0);
    ...
    fn test_performance_with_multiple_segments() {
        ...
        for segment in &segments {
            let _ = timeline_view.generate_pattern_preview(segment, 100.0, 20.0);
        }
