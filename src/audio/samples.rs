...
impl SampleBank {
    ...
    pub fn load_default_samples(&mut self) {
        ...
        self.add_sample(
            "kick".to_string(),
            Sample::from_data(kick_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "snare".to_string(),
            Sample::from_data(snare_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "hihat".to_string(),
            Sample::from_data(hihat_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "crash".to_string(),
            Sample::from_data(crash_data, sample_rate, 1),
        );

        // Open hi-hat - filtered noise with medium decay
        let open_hihat_data = generate_open_hihat(sample_rate as f32, 0.4);
        self.add_sample(
            "open_hihat".to_string(),
            Sample::from_data(open_hihat_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "clap".to_string(),
            Sample::from_data(clap_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "rimshot".to_string(),
            Sample::from_data(rimshot_data, sample_rate, 1),
        );
        ...
        self.add_sample(
            "tom".to_string(),
            Sample::from_data(tom_data, sample_rate, 1),
        );
    }
}
...
fn generate_clap(sample_rate: f32, duration: f32) -> Vec<f32> {
    ...
    for i in 0..samples {
        ...
        let burst2 = if t > 0.015 && t < 0.025 {
            (-(t - 0.015) * 200.0).exp()
        } else {
            0.0
        };
        let burst3 = if t > 0.03 && t < 0.04 {
            (-(t - 0.03) * 200.0).exp()
        } else {
            0.0
        };
        let main_envelope = if t > 0.045 {
            (-(t - 0.045) * 15.0).exp()
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_sample_bank_default_loading() {
        ...
        assert!(
            sample_bank.list_samples().is_empty(),
            "New sample bank should be empty"
        );
        ...
        assert!(
            !samples.is_empty(),
            "Sample bank should have samples after loading defaults"
        );
        ...
        for sample_name in expected_samples {
            assert!(
                samples.contains(&&sample_name.to_string()),
                "Should have sample: {}",
                sample_name
            );
            ...
            assert!(
                ...
                "Sample '{}' should have audio data",
                sample_name
            );
            assert!(
                sample.data.len() > 1000,
                "Sample '{}' should have reasonable length",
                sample_name
            );
            ...
            assert!(
                max_sample > 0.0,
                "Sample '{}' should have non-zero audio data",
                sample_name
            );
        }

        println!(
            "✅ Sample bank loads {} samples successfully",
            samples.len()
        );
    }
    ...
    fn test_sample_audio_characteristics() {
        ...
        assert!(
            initial_energy > 10.0,
            "Kick should have strong initial transient"
        );
        ...
        assert!(
            hihat.data.len() > 1000,
            "Hi-hat should have reasonable length"
        );
