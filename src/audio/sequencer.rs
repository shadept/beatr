...
impl TimeSignature {
    pub fn new(numerator: u8, denominator: u8) -> Result<Self, String> {
        ...
        if !denominator.is_power_of_two() || denominator == 0 || denominator > 32 {
            return Err(format!("Invalid denominator: {}. Must be power of 2 (1, 2, 4, 8, 16, 32)", denominator));
        }
    ...
    pub fn four_four() -> Self { TimeSignature { numerator: 4, denominator: 4 } }
    pub fn three_four() -> Self { TimeSignature { numerator: 3, denominator: 4 } }
    pub fn five_four() -> Self { TimeSignature { numerator: 5, denominator: 4 } }
    pub fn six_eight() -> Self { TimeSignature { numerator: 6, denominator: 8 } }
    pub fn seven_eight() -> Self { TimeSignature { numerator: 7, denominator: 8 } }
    pub fn nine_eight() -> Self { TimeSignature { numerator: 9, denominator: 8 } }
    pub fn twelve_eight() -> Self { TimeSignature { numerator: 12, denominator: 8 } }
...
impl Sequencer {
    ...
    pub fn process_audio_with_patterns(&mut self, output: &mut [f32], sample_bank: &SampleBank, patterns: &[Pattern]) {
        // Clear output buffer
    ...
    pub fn initialize_default_patterns(&mut self) {
        ...
        for (name, sample_name, active_steps) in patterns_data {
            let mut pattern = Pattern::new(name.to_string(), sample_name.to_string(), self.loop_length);
...
mod tests {
    ...
    fn test_sequencer_with_additional_samples() {
        ...
        let expected_names = vec!["Kick", "Snare", "Hi-Hat", "Crash", "Open Hi-Hat", "Clap", "Rim Shot", "Tom"];
    ...
    fn test_odd_time_signatures() {
        ...
        for &length in &odd_lengths {
            ...
            assert_eq!(sequencer.get_loop_length(), length, "Failed to set loop length to {}", length);
            
            // Test step advancement wraps correctly
            for _ in 0..length {
                sequencer.advance_step();
            }
            assert_eq!(sequencer.get_current_step(), 0, "Step should wrap to 0 after {} steps", length);
            
            // Test that patterns are resized correctly
            sequencer.initialize_default_patterns();
            let patterns = sequencer.get_patterns();
            for pattern in patterns {
                assert_eq!(pattern.length(), length, "Pattern should be resized to {}", length);
            }
        }
    }
    ...
    fn test_sequencer_time_signature_integration() {
        ...
        let patterns = sequencer.get_patterns();
        for pattern in patterns {
            assert_eq!(pattern.length(), 20, "Pattern should be resized to match new time signature");
        }
    }
    ...
    fn test_default_patterns_have_active_steps() {
        ...
        let active_steps: Vec<usize> = kick_pattern.steps.iter()
            .enumerate()
            .filter(|(_, step)| step.active)
            .map(|(i, _)| i)
            .collect();
        
        assert_eq!(active_steps, vec![0, 4, 8, 12], "Kick should be active on downbeats");
        ...
        let snare_active_steps: Vec<usize> = snare_pattern.steps.iter()
            .enumerate()
            .filter(|(_, step)| step.active)
            .map(|(i, _)| i)
            .collect();
        
        assert_eq!(snare_active_steps, vec![4, 12], "Snare should be active on beats 2 & 4");
        ...
        let hihat_active_steps: Vec<usize> = hihat_pattern.steps.iter()
            .enumerate()
            .filter(|(_, step)| step.active)
            .map(|(i, _)| i)
            .collect();
        
        assert_eq!(hihat_active_steps, vec![2, 6, 10, 14], "Hi-hat should be active on off-beats");
    ...
    fn test_audio_processing_with_active_patterns() {
        ...
        for i in 0..15 {  // Process 15 buffers (should advance ~2-3 steps)
            ...
            if step_after != step_before {
                step_triggers += 1;
                println!("Step advanced from {} to {} on buffer {}", step_before, step_after, i);
            }
        }
        ...
        assert!(step_triggers > 0, "Sequencer should advance steps during processing");
        assert!(total_max_sample > 0.0, "Audio processing should generate non-zero samples");
    ...
    fn test_voice_triggering() {
        ...
        assert!(active_voices > 0, "At least one voice should be active after triggering step 0");
        
        println!("✅ Voice triggering works - {} voices active", active_voices);
    }
    ...
    fn test_sequencer_timing_calculations() {
        ...
        assert!((sequencer.samples_per_step as f32 - expected_samples_per_step).abs() < 1.0, 
        ...
        for i in 0..20 {
            ...
            if i == 0 {
                println!("After buffer {}: step={}, counter={}", i, current_step, current_counter);
            }
            if current_step != initial_step {
                println!("✅ Step advanced from {} to {} after {} buffers", initial_step, current_step, i+1);
                return; // Success!
            }
        }
        
        println!("Final: step={}, counter={}, samples_per_step={}", 
                sequencer.get_current_step(), sequencer.sample_counter, sequencer.samples_per_step);
